...
class SwinUnet3D(nn.Module):
    def __init__(self, *, hidden_dim, layers, heads, in_channel=1, num_classes=2, head_dim=32,
                 ...
                 stl_channels: int = 8, final_test=True):  # second_to_last_channels
        super().__init__()
...
if __name__ == '__main__':
    ...
    net = SwinUnet3D(hidden_dim=96, layers=(2, 2, 6, 2), heads=(3, 6, 12, 24), window_size=[4, 4, 2],
                               in_channel=1, num_classes=1,
                               final_test=False, stl_channels=16)
