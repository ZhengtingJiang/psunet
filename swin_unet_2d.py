...
class CyclicShift2D(nn.Module):
    def __init__(self, displacement):
        super().__init__()

        assert type(displacement) is int or len(displacement) == 2, f'displacement must be 1 or 2 dimension'
        if type(displacement) is int:
            displacement = np.array([displacement, displacement])
...
def create_mask2D(window_size: Union[int, List[int]], displacement: Union[int, List[int]],
                  x_shift: bool, y_shift: bool):
    assert type(window_size) is int or len(window_size) == 2, f'window_size must be 1 or 2 dimension'
    ...
    mask = torch.zeros(window_size[0] * window_size[1] ,
                       window_size[0] * window_size[1])  # (wx*wy*wz, wx*wy*wz)
    mask = rearrange(mask, '(x1 y1) (x2 y2) -> x1 y1 x2 y2',
                     x1=window_size[0], y1=window_size[1], x2=window_size[0], y2=window_size[1])

    x_dist, y_dist = displacement[0], displacement[1]
...
class WindowAttention2D(nn.Module):
    def __init__(self, dim: int, heads: int, head_dim: int, shifted: bool, window_size: Union[int, List[int]],
        ...
        if self.shifted:
            ...
            self.cyclic_shift = CyclicShift2D(-displacement)
            self.cyclic_back_shift = CyclicShift2D(displacement)
            self.x_mask = nn.Parameter(create_mask2D(window_size=window_size, displacement=displacement,
                                                     x_shift=True, y_shift=False), requires_grad=False)
            self.y_mask = nn.Parameter(create_mask2D(window_size=window_size, displacement=displacement,
                                                     x_shift=False, y_shift=True), requires_grad=False)
    ...
    def forward(self, x):
        ...
        b, n_x, n_y, _, h = *x.shape, self.heads #
        ...
        q, k, v = map(
            lambda t: rearrange(t, 'b (nw_x w_x) (nw_y w_y) (h d) -> b h (nw_x nw_y) (w_x w_y) d',
                                h=h, w_x=self.window_size[0], w_y=self.window_size[1]), qkv)
        ...
        if self.shifted:
            ...
            dots = rearrange(dots, 'b h (n_x n_y) i j -> b h n_y n_x i j',
                             n_x=nw_x, n_y=nw_y)
            #   b h n_y n_x i j
            dots[:, :, -1, :, :, :] += self.x_mask

            dots = rearrange(dots, 'b h n_y n_x i j -> b h n_x n_y i j')
            dots[:, :, -1, :, :, :] += self.y_mask
        ...
        out = rearrange(out, 'b h (nw_x nw_y) (w_x w_y) d -> b (nw_x w_x) (nw_y w_y) (h d)',
                        h=h, w_x=self.window_size[0], w_y=self.window_size[1],
                        nw_x=nw_x, nw_y=nw_y)
        out = self.to_out(out)
...
class SwinBlock2D(nn.Module):  # 不会改变输入空间分辨率
    def __init__(self, dim, heads, head_dim, mlp_dim, shifted, window_size: Union[int, List[int]],
        ...
        super().__init__()
        self.attention_block = Residual2D(PreNorm2D(dim, WindowAttention2D(dim=dim,
                                                                           heads=heads,
                                                                           head_dim=head_dim,
                                                                           shifted=shifted,
                                                                           window_size=window_size,
                                                                           relative_pos_embedding=relative_pos_embedding)))
        self.mlp_block = Residual2D(PreNorm2D(dim, FeedForward2D(dim=dim, hidden_dim=mlp_dim, dropout=dropout)))
...
class PatchMerging2D(nn.Module):
    def __init__(self, in_dim, out_dim, downscaling_factor):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv2d(in_dim, out_dim, kernel_size=downscaling_factor, stride=downscaling_factor),
            Norm2D(dim=out_dim),
        )
    ...
    def forward(self, x):
        # x: B, C, H, W
        x = self.net(x)
        return x  # B,  H //down_scaling, W//down_scaling,  out_dim


class PatchExpanding2D(nn.Module):
    def __init__(self, in_dim, out_dim, up_scaling_factor):
        super(PatchExpanding2D, self).__init__()
...
class ConvBlock(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, kernel_size=3, stride=1, padding=1, groups=groups),
            Norm2D(dim=out_ch),
            nn.PReLU(),

            nn.Conv2d(out_ch, out_ch, kernel_size=3, stride=1, padding=1, groups=groups),
            Norm2D(dim=out_ch),
            nn.PReLU(),
        )
...
class Encoder2D(nn.Module):
    def __init__(self, in_dims, hidden_dimension, layers, downscaling_factor, num_heads, head_dim,
        ...
        self.patch_partition = PatchMerging2D(in_dim=in_dims, out_dim=hidden_dimension,
        ...
        for _ in range(layers // 2):
            self.swin_layers.append(nn.ModuleList([
                SwinBlock2D(dim=hidden_dimension, heads=num_heads, head_dim=head_dim, mlp_dim=hidden_dimension * 4,
...
class Decoder2D(nn.Module):
    def __init__(self, in_dims, out_dims, layers, up_scaling_factor, num_heads, head_dim,
        ...
        self.patch_expand = PatchExpanding2D(in_dim=in_dims, out_dim=out_dims,
        ...
        for _ in range(layers // 2):
            self.swin_layers.append(nn.ModuleList([
                SwinBlock2D(dim=out_dims, heads=num_heads, head_dim=head_dim, mlp_dim=out_dims * 4,
                            shifted=False, window_size=window_size, relative_pos_embedding=relative_pos_embedding,
                            dropout=dropout),
                SwinBlock2D(dim=out_dims, heads=num_heads, head_dim=head_dim, mlp_dim=out_dims * 4,
                            shifted=True, window_size=window_size, relative_pos_embedding=relative_pos_embedding,
...
class SwinUnet2D(nn.Module):
    def __init__(self, *, hidden_dim, layers, heads, in_channel=1, num_classes=2, head_dim=32,
                 window_size: Union[int, List[int]] = 8, downscaling_factors=(4, 2, 2, 2),
                 relative_pos_embedding=True, dropout: float = 0.0, skip_style='stack',
                 stl_channels: int = 16, final_test=False):  # second_to_last_channels
        super().__init__()

        self.dsf = downscaling_factors
        self.window_size = window_size

        self.enc12 = Encoder2D(in_dims=in_channel, hidden_dimension=hidden_dim, layers=layers[0],
                             downscaling_factor=downscaling_factors[0], num_heads=heads[0],
                             head_dim=head_dim, window_size=window_size, dropout=dropout,
                             relative_pos_embedding=relative_pos_embedding)
        self.enc3 = Encoder2D(in_dims=hidden_dim, hidden_dimension=hidden_dim * 2,
                            layers=layers[1],
                            downscaling_factor=downscaling_factors[1], num_heads=heads[1],
                            head_dim=head_dim, window_size=window_size, dropout=dropout,
                            relative_pos_embedding=relative_pos_embedding)
        self.enc4 = Encoder2D(in_dims=hidden_dim * 2, hidden_dimension=hidden_dim * 4,
                            layers=layers[2],
                            downscaling_factor=downscaling_factors[2], num_heads=heads[2],
                            head_dim=head_dim, window_size=window_size, dropout=dropout,
                            relative_pos_embedding=relative_pos_embedding)
        self.enc5 = Encoder2D(in_dims=hidden_dim * 4, hidden_dimension=hidden_dim * 8,
                            layers=layers[3],
                            downscaling_factor=downscaling_factors[3], num_heads=heads[3],
                            head_dim=head_dim, window_size=window_size, dropout=dropout,
                            relative_pos_embedding=relative_pos_embedding)

        self.dec4 = Decoder2D(in_dims=hidden_dim * 8, out_dims=hidden_dim * 4,
                            layers=layers[2],
                            up_scaling_factor=downscaling_factors[3], num_heads=heads[2],
                            head_dim=head_dim, window_size=window_size, dropout=dropout,
                            relative_pos_embedding=relative_pos_embedding)

        self.dec3 = Decoder2D(in_dims=hidden_dim * 4, out_dims=hidden_dim * 2,
                            layers=layers[1],
                            up_scaling_factor=downscaling_factors[2], num_heads=heads[1],
                            head_dim=head_dim, window_size=window_size, dropout=dropout,
                            relative_pos_embedding=relative_pos_embedding)

        self.dec12 = Decoder2D(in_dims=hidden_dim * 2, out_dims=hidden_dim,
                             layers=layers[0],
    ...
    def forward(self, img):
        ...
        assert type(window_size) is int or len(window_size) == 2, f'window_size must be 1 or 3 dimension'
        if type(window_size) is int:
            window_size = np.array([window_size, window_size, window_size])
        _, _, x_s, y_s = img.shape
        x_ws, y_ws = window_size

        assert x_s % (x_ws * 32) == 0, f'x轴上的尺寸必须能被x_window_size*32 整除'
        assert y_s % (y_ws * 32) == 0, f'y轴上的尺寸必须能被y_window_size*32 整除'

        down12_1 = self.enc12(img)  # (B,C, X//4, Y//4)
        down3 = self.enc3(down12_1)  # (B, 2C,X//8, Y//8)
        down4 = self.enc4(down3)  # (B, 4C,X//16, Y//16)
        features = self.enc5(down4)  # (B, 8C,X//32, Y//32)

        up4 = self.dec4(features)  # (B, 8C, X//16, Y//16)
        # up1和 down3融合
        up4 = self.converge4(up4, down4)  # (B, 4C, X//16, Y//16)

        up3 = self.dec3(up4)  # ((B, 2C,X//8, Y//8)
        # up2和 down2融合
        up3 = self.converge3(up3, down3)  # (B,2C, X//8, Y//8)

        up12 = self.dec12(up3)  # (B,C, X//4, Y//4)
        # up3和 down1融合
        up12 = self.converge12(up12, down12_1)  # (B,C, X//4, Y//4)

        out = self.final(up12)  # (B,num_classes, X, Y)
        if self.final_test:
            out = self.out(out)
            return out
        return out
...
if __name__ == "__main__":
    ...
    model = SwinUnet2D(hidden_dim=96, layers=(2, 2, 6, 2), heads=(3, 6, 12, 24),
                                    window_size=window_size, in_channel=1, num_classes=1).to(device)
    x = x.to(device)
    output = model(x)
    print(window_size)
    print(x.shape)
